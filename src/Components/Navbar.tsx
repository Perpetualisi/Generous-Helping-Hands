...
const Logo: React.FC<{ onClick: () => void; compact: boolean }> = ({ onClick, compact }) => (
  <button onClick={onClick} aria-label="Go to home" style={{
    ...
    <div style={{
      ...
      boxShadow: "0 0 0 1px rgba(245,158,11,0.25), 0 4px 18px rgba(245,158,11,0.2)",
    }}>
    ...
    <div style={{ position: "relative" }} onMouseEnter={show} onMouseLeave={hide}>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div style={{ height: 1, margin: "0 0.6rem 0.5rem", background: `linear-gradient(90deg,${C.gold},transparent)` }} />
            {item.subLinks.map(sub => (
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.color = C.gold; e.currentTarget.style.background = "rgba(245,158,11,0.07)"; e.currentTarget.style.paddingLeft = "1.25rem"; }}
                onMouseLeave={e => { e.currentTarget.style.color = C.textMuted; e.currentTarget.style.background = "transparent"; e.currentTarget.style.paddingLeft = "1rem"; }}
              >{sub.name}</button>
            ))}
          </motion.div>
        )}
      ...
      >
        ...
        {item.subLinks && (
          <motion.div animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.2 }}>
            <ChevronDown size={16} color={isOpen ? C.gold : "rgba(255,255,255,0.28)"} />
          </motion.div>
        )}
      </button>
      <AnimatePresence>
        {isOpen && item.subLinks && (
          <motion.div
            ...
            style={{ overflow: "hidden", borderLeft: "2px solid rgba(245,158,11,0.2)", marginLeft: "0.3rem" }}
          >
            {item.subLinks.map(sub => (
              <button key={sub.href} onClick={() => onNavigate(sub.href)}
                ...
                onMouseEnter={e => { e.currentTarget.style.color = C.gold; e.currentTarget.style.paddingLeft = "1.6rem"; }}
                onMouseLeave={e => { e.currentTarget.style.color = C.textMuted; e.currentTarget.style.paddingLeft = "1.2rem"; }}
              >{sub.name}</button>
            ))}
          </motion.div>
        )}
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        /* Hide announcement contacts on narrow screens */
        @media (max-width: 540px) { .ann-contact { display: none !important; } }
      ...
      <motion.div
        ...
        style={{
          ...
          gap: "1.5rem", overflow: "hidden", padding: "0 1rem",
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ...
          <span style={{
            ...
            Empowering Women &amp; Girls Across Africa
          </span>
          <Heart size={10} fill="white" color="white" />
        </div>

        <a href={`mailto:${EMAIL}`} className="ann-contact" style={{
          display: "flex", alignItems: "center", gap: "0.4rem",
          color: "rgba(255,255,255,0.75)", textDecoration: "none",
          fontFamily: "'DM Sans', sans-serif",
          fontSize: "0.54rem", fontWeight: 600, letterSpacing: "0.12em",
      ...
      <nav className="nav-shell" style={{
        ...
        boxShadow: scrolled ? "0 8px 50px rgba(0,0,0,0.55), inset 0 1px 0 rgba(255,255,255,0.03)" : "none",
      }}>
        <div style={{
          maxWidth: 1440, margin: "0 auto",
          padding: "0 4%", height: "100%",
          display: "flex", alignItems: "center", justifyContent: "space-between",
          gap: "0.75rem",
        }}>
          ...
          {/* ── DESKTOP NAV LINKS ── */}
          <div className="dsk-links" style={{ display: "none", alignItems: "center", gap: "0.05rem", flex: 1, justifyContent: "center" }}>
            {MENU_ITEMS.map(item => (
              <DesktopDropdown
          ...
          {/* ── DESKTOP RIGHT — supporters counter removed ── */}
          <div className="dsk-right" style={{ display: "none", alignItems: "center", gap: "0.6rem", flexShrink: 0 }}>
            <motion.button
          ...
          >
            <div style={{
              ...
              <p style={{
                ...
                color: C.gold, marginTop: "1.25rem", marginBottom: "0.4rem",
              }}>Main Menu</p>
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  ...
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.52rem", fontWeight: 700, letterSpacing: "0.3em", textTransform: "uppercase", color: C.gold }}>
                    Quick Contact
                  </p>
                  {([
                    { icon: Phone, label: PHONE,            href: "tel:+2349036854354" },
                    ...
                    <div key={idx} style={{ display: "flex", alignItems: "center", gap: "0.65rem" }}>
                      <div style={{ width: 32, height: 32, borderRadius: 9, flexShrink: 0, background: "rgba(245,158,11,0.08)", border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <Icon size={13} color={C.gold} />
                      </div>
                      {href ? (
                        <a href={href} style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.82rem", color: C.textMuted, textDecoration: "none", transition: "color 0.2s", wordBreak: "break-all" }}
                          onMouseEnter={e => e.currentTarget.style.color = C.gold}
                          onMouseLeave={e => e.currentTarget.style.color = C.textMuted}
                        >{label}</a>
                      ) : (
                        <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.82rem", color: C.textMuted }}>{label}</span>
                      )}
                    </div>
                  ))}
                </motion.div>
